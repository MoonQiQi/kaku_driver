...
public class ECCallHeadUILayout extends LinearLayout implements View.OnClickListener {
    ...
    public void setCalling(boolean calling) {
        ...
        if (calling) {
            ...
            recordingTime = SystemClock.elapsedRealtime() - mCallTime.getBase();// 保存这次记录了的时间
            SharedPreferences.Editor editor = KaKuApplication.editor;
            editor.putFloat(Constants.CALLTIME, recordingTime / 1000);
            editor.commit();
        }
    ...
    public void setCallNumber(CharSequence mobile) {
        if (mCallNumber != null) {
            ...
            /*if (mobile != null) {
                ECContacts mContacts = getContact(mobile.toString());
