...
public class BitmapUtil {
	...
	public static void download(ImageView img, String url) {
		if (TextUtils.isEmpty(url)) {
			return;
		}
		utils.display(img, url);
	}

	public static void init() {
		config.setLoadFailedDrawable(context.getResources().getDrawable(R.drawable.meidian));
	}
    ...
    public static String bitmap2Str(Bitmap bitmap) {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        bitmap.compress(Bitmap.CompressFormat.JPEG, 100, baos);//bitmap压缩到流里面
        return new String(Base64Coder.encodeLines(baos.toByteArray()));
    }
